...
public class ExportAction implements IAction {
	...
	public String execute(HttpServletRequest request,
		...
		// ----- Check if User is logged in correctly + check if user is allowed
		// to use action ----- //
		...
		if (loginBean.isLoggedIn() && loginBean != null) {
			...
			ExportHandler exportHandler = new ExportHandler(shortDescription,
					fileType);
	...
	public boolean accepts(String actionName) {
		return actionName.equalsIgnoreCase(Constants.ACTION_EXPORT_CATALOG);
	}
